...
We're not sure if its a Right Triangle. In this case we no NO sides, and we know
all 3 sides. This means we only need to use the Law Of Cosines solving for the
Angle, not for the Side.
...
sides, or all three angles), to solve these sorts 9f problems. We only have one
